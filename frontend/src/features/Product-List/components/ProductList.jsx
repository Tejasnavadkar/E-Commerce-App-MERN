...
const ProductList = () => {
  ...
  const handleFilter = (e, section, option) => {
    ...
    if (isChecked) {
      // agar filter uncheck kiya to firse reset ho jayega
      ...
      // when uncheck remove from newFilter
              ...
              {products.length > 0 ? (
                products.map((product) => (
                  <Link
                    ...
                    key={product.id}
                    className=" group relative border-1 border-gray-500 p-2"
                  >
                    {" "}
                    {/*onClick={() => navigate(`/product-details`,{state:{id:product.id}})}*/}
                    <div className="">
                      <img
                        alt={"image not found"}
